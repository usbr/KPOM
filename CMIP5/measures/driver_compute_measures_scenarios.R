...
for(scen in 1:nscen){
	...
	#						by WATER YEAR
	# 						REPLACED WITH compute mean annual water deliveries to refuge by WATER YEAR
	#======================================================
...
#	refuge.mean.percent.desired.supply <- refuge.mean.ann.supply/95400*100
	refuge.mean.percent.desired.supply <- refuge.mean.ann.supply/1000
	names(refuge.mean.percent.desired.supply) <- "Perc_tot_supply"
	#print(refuge.mean.percent.desired.supply)

	# write output
	xx <- cbind(sprintf("%25s","Mean Annual Water Delivery to LKNWR\t"),
	...
	xx <- cbind(sprintf("%25s","Copco1 Mean Spill Days per Year\t"),
					...
					sprintf("%12s","days"))
	write.table(xx,fout,row.names=F,col.names=F,sep=" ",append=TRUE,quote=FALSE)
	xx <- cbind(sprintf("%25s","Iron Gate Mean Spill Days per Year\t"),
...
#	avgperiod <- as.Date(as.character(as.POSIXct(c(tmp[1,1], tmp[length(tmp[,1]),1]))))
	...
	#======================================================
	# WATER DELIVERY MEASURE 1: Klamath Project Supply
	#                           take average monthly value from April-September
	#                           to compare with full supply of 390KAF
	#======================================================
...
#	t2							<- aggdata[,2]/fullsupply*100
	...
	xx <- cbind(sprintf("%25s","Mean Project Supply (Apr-Sep)\t"),sprintf("%12.4f\t",ProjectPercentFullSupply.mean),sprintf("%12s","KAF"))
	write.table(xx,fout,row.names=F,col.names=F,sep=" ",append=TRUE,quote=FALSE)
