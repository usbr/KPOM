...
#Model Outputs
Model = read.xlsx(paste(Sys.getenv("KLAMATH_OPS"),"/Testing/Testing Output.xlsx", sep = ""), 
                  sheet=1, colNames=TRUE, cols=2:18, na.strings = "NaN")
#IGD Calculator Outputs
IGD = read.xlsx(paste(Sys.getenv("KLAMATH_OPS"),"/Testing/IGD Linked Worksheet.xlsx", sep = ""), 
...
suppressWarnings(
  for (i in 2:18){
    z <- ggplot() + geom_point(data = diffdf[1:obsdays,], aes(x = Dates, y = diffdf[1:obsdays,i], color = "a"), 
      ...
      ylab(paste(Names[(i-1)],"Difference","(", Units[(i-1)], ")", sep = " ")) +
