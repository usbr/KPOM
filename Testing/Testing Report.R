...
suppressWarnings(
for (i in 2:18){
  z <- ggplot() + geom_point(data = diffdf[1:obsdays,], aes(x = Dates, y = diffdf[1:obsdays,i], color = diffdf[1:obsdays,i])) +
    scale_color_gradient(name = "Observed", low = "orange", high = "orangered2") +
    geom_point(data = diffdf[obsdays:NumData,], aes(x = Dates, y = diffdf[obsdays:NumData,i], fill = diffdf[obsdays:NumData,i]), shape = 21) +
    scale_fill_gradient(name = "Projected", low = "purple", high = "light blue") +
